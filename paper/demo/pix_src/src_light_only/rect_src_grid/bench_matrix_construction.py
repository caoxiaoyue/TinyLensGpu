"""Benchmark matrix-construction performance for rectangular pixelized source models.
...
def _parse_scheme_list(text: str) -> List[str]:
    """Parse and normalize rectangular regularization schemes."""
    out: List[str] = []
    for token in text.split(","):
        ...
        if scheme:
            out.append(scheme)
    if not out:
        raise ValueError("No rectangular regularization schemes were provided.")
    return out


def _parse_grid_shapes(text: str) -> List[Tuple[int, int]]:
    """Parse a comma-separated list of rectangular grid shapes ``NxM``."""
    ...
    for token in text.split(","):
        ...
        if "x" not in token:
            raise ValueError(
                f"Invalid grid shape token '{token}', expected format NxM (e.g. 64x64)."
            )
        sx, sy = token.split("x", 1)
        nx = int(sx)
        ny = int(sy)
        if nx <= 0 or ny <= 0:
            raise ValueError(f"Grid shape values must be positive, got '{token}'.")
        out.append((nx, ny))
    if not out:
...
def _load_data_from_fits(*, default_dpix: float = 0.05) -> Dict[str, Any]:
...
def _build_prob_model(
    ...
    *,
    ...
    """Construct a matrix-backend probability model for one rectangular-grid case."""
...
def _best_by_metric(cases: List[Dict[str, Any]], metric_name: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "source_grid_nx": int(best["source_grid_nx"]),
        "source_grid_ny": int(best["source_grid_ny"]),
...
def _run_benchmark(
    *,
    ...
    """Run all rectangular-grid matrix-construction benchmark cases."""
    data_dict = _load_data_from_fits(default_dpix=0.05)
    cases: List[Dict[str, Any]] = []

    for nx, ny in grid_shapes:
        for scheme in schemes:
            print(f"[Case] source_grid={nx}x{ny}, scheme={scheme}, backend=matrix")
            ...
            reg_stats, reg_warmup = _time_callable(
                lambda: prob_model.simulator.build_regularization_matrix(
                    reg_scale=reg_scale,
                    reg_coefficient=reg_coeff,
                ),
                n_runs=n_runs,
            )
            ...
            cases.append(
                {
                    "backend": "matrix",
                    "source_grid_nx": int(nx),
                    "source_grid_ny": int(ny),
    ...
    return {
        "benchmark_name": "rect_matrix_construction",
        "backend": "matrix",
        "data_source": "fits",
        "n_runs": int(n_runs),
        ...
        "nonnegative": bool(nonnegative),
        "grid_shapes": [[int(nx), int(ny)] for (nx, ny) in grid_shapes],
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark mapping/regularization matrix construction speed (rectangular grid, matrix backend)."
    )
    parser.add_argument("--n-runs", type=int, default=5)
    parser.add_argument("--grid-shapes", type=str, default="32x32,48x48,64x64,80x80")
    parser.add_argument("--schemes", type=str, default="rectangular_zero,rectangular_first,rectangular_second")
...
def main() -> None:
    ...
    if float(args.source_grid_margin_frac) < 0.0:
        raise ValueError("--source-grid-margin-frac must be >= 0.")
