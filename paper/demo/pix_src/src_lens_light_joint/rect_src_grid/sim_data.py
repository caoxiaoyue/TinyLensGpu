"""Simulate lens+source+lens-light imaging data for rectangular-grid inversion demo.

This script keeps simulation-time source/lens-light components as Sersic profiles,
while the corresponding inversion demo models lens light with an MGE basis.
"""
...
def build_physical_model() -> PhysicalModel:
    """Build simulation-time mass/source/lens-light model components."""
...
def add_mock_noise(ideal_image: np.ndarray, *, back_rms: float, exp_time: float, seed: int) -> tuple[np.ndarray, np.ndarray]:
    """Add Gaussian pixel noise from a Poisson+background approximation."""
...
def simulate_lensing_data(seed: int = 0) -> dict[str, np.ndarray | float]:
    """Run end-to-end simulation and return the generated arrays."""
    dpix = 0.074
    npix = 200
...
def main() -> None:
    """Generate FITS data products for rectangular-grid joint inversion demo."""
