"""
This script performs a lens-only model fitting where all parameters, including the light amplitude (Ie), 
are solved non-linearly using the Nautilus sampler.

Key Differences from the standard lens_only run:
- Optimization: The light amplitude (Ie) is NOT solved linearly. 
- Priors: A log-uniform prior is placed on Ie, which is then sampled directly alongside other parameters.
- Inference: Evaluates the performance of the sampler when handling more complex parameter spaces without linear shortcuts.
"""
