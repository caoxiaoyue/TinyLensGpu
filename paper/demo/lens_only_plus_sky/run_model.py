"""
This script performs a parametric lens-light fit that includes both a Sersic
profile and a constant sky background.

Key Features:
- Model: Single Sersic profile plus a constant background component.
- Optimization: Uses a linear solver (NNLS) to determine both the Sersic
  amplitude (Ie) and the sky background amplitude at each sampling step.
- Inference: Utilizes the Nautilus nested sampler with vectorized likelihood
  evaluations for improved performance.
- Output: Generates posterior samples, parameter summaries, and visualization
...
if __name__ == "__main__":
    ...
    sky_background = ConstantBackground(intensity=ParamU("sky_intensity", 1.0))
    ...
    sky_background.intensity.to_static(1.0)  # Will be solved linearly
    ...
    likelihood = ImageProbModel(
        ...
        use_linear=True,  # Use linear solver for Ie and sky intensity
        mask=mask,
        solver_type='nnls',
    )
