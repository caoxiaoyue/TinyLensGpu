...
if __name__ == "__main__":
    ...
    # Define dynamic ellipticity parameters
    e1_param = ParamU("e1", 0.0, prior_type="gaussian", prior_settings=[0.0, 0.3], limits=[-1.0, 1.0])
    e2_param = ParamU("e2", 0.0, prior_type="gaussian", prior_settings=[0.0, 0.3], limits=[-1.0, 1.0])
    
    # Create the constraint module (requires q >= 0.5)
    q_constraint = EllipticityConstraint(e1=e1_param, e2=e2_param, q_min=0.3, phi_min=70, phi_max=95.0)
    ...
    lens_light = SersicEllipse(
        ...
        n_sersic=ParamU("n_sersic", 4.0,
                        ...
                        limits=[0.3, 6.0]),
        e1=ck.Param("e1", lambda p: q_constraint.out_e1()),
        e2=ck.Param("e2", lambda p: q_constraint.out_e2()),
        center_x=ParamU("center_x", 0.0),
        center_y=ParamU("center_y", 0.0),
        Ie=ParamU("Ie", 1.0),
    )
    
    # Attach the constraint module as a child so that `extract_prior_specs` can find `e1_raw` and `e2_raw`
    lens_light.q_constraint = q_constraint
    
    # Set parameters to dynamic for sampling
    lens_light.R_sersic.to_dynamic()
    lens_light.n_sersic.to_dynamic()
    q_constraint.e1_raw.to_dynamic()
    q_constraint.e2_raw.to_dynamic()
    ...
    if e1_idx != -1 and e2_idx != -1:
        ...
        PA_med = float(phi_med) * 180.0 / np.pi
        PA_med = jnp.mod(PA_med, 180.0)
        print(f"  {'q (derived)':12s} = {float(q_med):.4f}")
        print(f"  {'PA (derived)':12s} = {PA_med:.4f} deg")
