"""
Lens + Source model with magnitude-based brightness (Non-linear mode).
"""
...
if __name__ == "__main__":
    print("="*60)
    print("Lens + Source Model Inference (Magnitude-based, Non-linear)")
    print("="*60)
    ...
    source = SersicEllipse(
        ...
        center_y=ParamU("center_y_src", 0.0, prior_type="gaussian", 
                        prior_settings=[0.0, 0.5], limits=[-3.0, 3.0]),
        magnitude=ParamU("magnitude_src", 22.0, prior_type="gaussian",
                         prior_settings=[22.0, 1.0], limits=[18.0, 26.0]),
        mag_zero_point=22.0,
    )
    ...
    lens = SersicEllipse(
        ...
        magnitude=ParamU("magnitude_lens", 20.0, prior_type="gaussian",
                         prior_settings=[20.0, 1.0], limits=[16.0, 24.0]),
        mag_zero_point=22.0,
    )
    ...
    source.magnitude.to_dynamic()
    lens.R_sersic.to_dynamic()
    lens.n_sersic.to_dynamic()
    lens.e1.to_dynamic()
    lens.e2.to_dynamic()
    lens.magnitude.to_dynamic()
