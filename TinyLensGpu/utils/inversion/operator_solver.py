...
def _lanczos_logdet(matvec, n_dim: int, *, seed: int, probes: int, steps: int) -> Array:
    def lanczos_one(z: Array) -> Array:
        ...
        def body(carry, _):
            ...
            w = matvec(q_k) - beta_prev_k * q_prev_k
            alpha = jnp.dot(q_k, w)
            w = w - alpha * q_k
...
class OperatorInversion:
    ...
    def __init__(
        ...
        *,
        ...
        self.psf_fft = jnp.asarray(psf_fft)
    ...
    @jit
    def model_predict(self, s: Array) -> Array:
        forward, _ = self._ops()
        return forward(jnp.asarray(s, dtype=jnp.float32))

    @jit
    def objective_value(self, s: Array) -> Array:
        _, n_inv = _safe_noise_inverse(self.noise_var)
        model = self.model_predict(s)
        resid = self.d - model
        chi2 = jnp.sum((resid * resid) * n_inv)
        reg = jnp.dot(s, self._apply_H(s)) + self.jitter * jnp.dot(s, s)
        return chi2 + reg
    ...
    def tree_unflatten(cls, aux_data, children):
        ...
        return cls(
            ...
            H_sparse_rows=H_sparse_rows,
            H_sparse_cols=H_sparse_cols,
            H_sparse_values=H_sparse_values,
