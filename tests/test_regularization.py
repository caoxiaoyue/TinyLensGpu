...
class TestDenseRegularizationBuilder:
    ...
    @pytest.mark.parametrize(
        "regularization_type",
        ["zero", "first", "curvature", "exponential", "gaussian"],
    )
    ...
    def test_zero_regularization_is_identity(self, small_source_grid_shape):
        ...
        nx, ny = small_source_grid_shape
        builder = DenseRegularizationBuilder(nx, ny, "zero")
...
class TestTraditionalRegularizationScaling:
    ...
    def test_first_regularization_scales_with_inverse_spacing_squared(self, small_source_grid_shape):
        """Test first-gradient matrix scaling when physical grid spacing changes."""
        nx, ny = small_source_grid_shape
        builder = DenseRegularizationBuilder(nx, ny, "first")

        matrix_half_size_1 = builder.matrix(1.0)
        matrix_half_size_2 = builder.matrix(2.0)

        # first: H = (D/dx).T @ (D/dx), so doubling dx divides H by 4.
        assert jnp.allclose(matrix_half_size_2, matrix_half_size_1 / 4.0, rtol=1e-5, atol=1e-6)

    def test_curvature_regularization_scales_with_inverse_spacing_fourth(self, small_source_grid_shape):
        """Test curvature matrix scaling when physical grid spacing changes."""
        nx, ny = small_source_grid_shape
        builder = DenseRegularizationBuilder(nx, ny, "curvature")

        matrix_half_size_1 = builder.matrix(1.0)
        matrix_half_size_2 = builder.matrix(2.0)

        # curvature: H = (L/dx^2).T @ (L/dx^2), so doubling dx divides H by 16.
