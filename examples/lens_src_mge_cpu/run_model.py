...
# Restrict this process and the worker threads subsequently created by XLA to
# eight CPUs.  Select from the CPUs already allowed by the parent process so
# this also works under task schedulers and inside containers.
available_cpus = sorted(os.sched_getaffinity(0))
if len(available_cpus) < NCPUS:
    raise RuntimeError(
        f"Requested {NCPUS} CPUs, but only {len(available_cpus)} are available"
    )
CPU_AFFINITY = set(available_cpus[:NCPUS])
os.sched_setaffinity(0, CPU_AFFINITY)

os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = "false"
# os.environ["NPROC"] = str(NCPUS) #https://github.com/jax-ml/jax/issues/743
