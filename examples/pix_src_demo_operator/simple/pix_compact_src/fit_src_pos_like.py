...
with ck.ActiveContext(prob_model):
    ...
    N_eff = float(pix_src.n * pix_src.n - lam * trace_invPR)
...
plt.suptitle(
    ...
    f"(θ_E={q50_list[0]:.3f}\", e1={q50_list[1]:.3f}, λ={float(jnp.exp(lam_med)):.2e})",
    fontsize=12,
)
