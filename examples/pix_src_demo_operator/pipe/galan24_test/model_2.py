...
Stage m1 : SIE + shear + non-adaptive pixelized source — Nautilus sampling
            of mass parameters with lambda_reg fixed from stage-M0, then
            builds the fixed S1 source template
Stage m2 : fixed SIE + shear + adaptive pixelized source — Nautilus sampling
...
``output_adpt_reg_m1sie/stage_{a,m0,m1,m2,m3}.pkl`` and is re-runnable via ``--skip-done``.

Usage::

    # From galan24_test/
    python model_2.py
    python model_2.py --skip-done
    python model_2.py --skip-done --out-dir output_adpt_reg_fista1000
"""
...
PIPELINE_ID = "m0raw_m1sie_m2reg_m3epl"
...
OUT_DIR = Path("output_adpt_reg_m1sie")
DATA_DIR = Path("data")
...
def _fista_config():
    return dict(_fista_kwargs())


def _cache_matches_fista_config(stage_payload, tag, rtol=1.0e-12):
    expected = _fista_config()
    actual = stage_payload.get("extra", {}).get("fista_config")
    if actual is None:
        print(
            f"[stage-{tag.upper()}] cached output is missing FISTA config; "
            "recomputing."
        )
        return False
    for name, expected_value in expected.items():
        ...
        if actual_value is None or not np.isclose(
            float(actual_value), float(expected_value), rtol=rtol, atol=rtol,
        ):
            print(
                f"[stage-{tag.upper()}] cached output uses stale {name}; "
                "recomputing."
            )
            return False
    return True
...
def _cache_matches_reg_hyperparams(stage_payload, reg_hyperparams, tag, rtol=1.0e-8):
    ...
    if actual is None:
        actual = stage_payload.get("extra", {}).get("reg_hyperparams_reference")
    if actual is None:
        actual = {
...
def _sie_mass_from_stage_a_prior(passer: GaussianPriorPasser):
...
def _sie_mass_from_medians(medians: dict):
    """SIE (+ shear) with all parameters fixed at posterior medians."""
    required = (
        "theta_E",
        "e1_mass",
        "e2_mass",
    ...
    missing = [name for name in required if name not in medians]
    if missing:
        raise KeyError("SIE medians missing required keys: " + ", ".join(missing))
...
# ------------------------------------------------------------------ #
# Stage M1 — SIE + shear + non-adaptive pixelized source  (fit mass)
# ------------------------------------------------------------------ #
def build_stage_m1_likelihood(
    ...
    """Build M1: SIE+shear free, non-adaptive source lambda fixed from M0."""
    sie, shear = _sie_mass_from_stage_a_prior(passer)
...
def run_stage_m1(image_data, noise_map, psf_kernel, feature_mask,
    ...
    """Stage M1: fit SIE+shear with non-adaptive source lambda fixed from M0.

    ``image_data`` is already lens-subtracted so we use it directly.
    """
    print("\n" + "=" * 60)
    print(" Stage M1 : SIE + shear + non-adaptive pix source (mass fit)")
    print("=" * 60)
    print(
...
# ------------------------------------------------------------------ #
# Stage M2 — fixed SIE + shear + adaptive pixelized source  (fit source reg)
# ------------------------------------------------------------------ #
...
def build_stage_m2_likelihood(
    ...
    sie, shear = _sie_mass_from_medians(medians_m1)
    log_lam = ParamU(
        "log_lambda_reg",
...
def run_stage_m2(image_data, noise_map, psf_kernel, feature_mask,
    ...
    """
    print("\n" + "=" * 60)
    print(" Stage M2 : fixed SIE + shear + adaptive pix source (fit λ, rho)")
    print("=" * 60)
    t0 = time.time()
