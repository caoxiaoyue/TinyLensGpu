...
Usage::
    ...
    # In this directory (euclid_examples/)
...
if META_PATH.exists():
    with open(META_PATH, "r", encoding="utf-8") as fp:
        _META = json.load(fp)
    DPIX = float(_META.get("pixel_scale_arcsec", 0.1))
    LENS_ID = str(_META.get("lens_id", "unknown"))
    BAND = str(_META.get("band", "VIS"))
else:
    DPIX = 0.1
    LENS_ID = "unknown"
    BAND = "VIS"
...
def _unmasked_extent(mask, dpix):
    ...
    c0, c1 = int(cols_unmasked.min()), int(cols_unmasked.max())
    x0 = (c0 - (nx - 1) / 2) * dpix
    x1 = (c1 - (nx - 1) / 2) * dpix
    y0 = (r0 - (ny - 1) / 2) * dpix
    y1 = (r1 - (ny - 1) / 2) * dpix
...
def main(skip_done: bool = False, out_dir: str | None = None):
    ...
    image_data, noise_map, psf_kernel, bad_pixel_mask = load_lens_data(
        ...
        mask_path=str(DATA_DIR / "mask.fits"),
    )

    print("=" * 60)
    print(f" Euclid Q1 lens  |  id = {LENS_ID}  |  band = {BAND}")
    print(f" image  shape = {tuple(image_data.shape)}   dpix = {DPIX:.4f} arcsec/px")
    ...
    print(f"[base mask] excluded {n_excl} / {circular_mask.size} pixels "
          f"(circular r>{MASK_RADIUS}\" + bad pixels)")
