...
def _run_sampler(likelihood, n_live: int, n_eff: int, tag: str, vectorized: bool = True):
    ...
    t1 = time.time()
    print(f"[{tag}] sampling took {t1 - t0:.2f} seconds")
...
def _plot_pix_stage(tag, likelihood, medians, param_names, save_path):
    ...
    # Calculate tight bounding box for unmasked pixels
    y_idx, x_idx = np.where(~mask)
    if len(x_idx) > 0:
        x_min_p = ext_i[0] + x_idx.min() * DPIX
        x_max_p = ext_i[0] + (x_idx.max() + 1) * DPIX
        y_min_p = ext_i[2] + y_idx.min() * DPIX
        y_max_p = ext_i[2] + (y_idx.max() + 1) * DPIX
    else:
        x_min_p, x_max_p, y_min_p, y_max_p = ext_i

    fig, axes = plt.subplots(1, 4, figsize=(17, 4.2))
    for ax, img, title, kw in [
        (axes[0],  np.where(mask, np.nan, image_data),  "Data (lens-subtracted)", dict(vmin=0, vmax=vmax, cmap="viridis")),
        (axes[1], np.where(mask, np.nan, model_image), "Model image",            dict(vmin=0, vmax=vmax, cmap="viridis")),
...
# ------------------------------------------------------------------ #
def _plot_m1_grid(likelihood, lam_coarse, log_ev_coarse,
                   lam_fine, log_ev_fine, lam_best, log_ev_best):
    """2-panel diagnostic: log-evidence vs lambda_reg for both grid stages."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 4.5))

    ax1.semilogx(np.asarray(lam_coarse), np.asarray(log_ev_coarse), 'b.-', markersize=2)
    ax1.axvline(lam_best, color='r', linestyle='--', alpha=0.8,
                label=f'best λ = {lam_best:.3e}')
    ax1.set_xlabel('λ_reg'); ax1.set_ylabel('log-evidence')
    ax1.set_title('Stage M1: Coarse grid (200 pts, 1e-6 – 1e6)')
    ax1.legend(); ax1.grid(True, alpha=0.3)

    ax2.semilogx(np.asarray(lam_fine), np.asarray(log_ev_fine), 'b.-', markersize=2)
    ax2.axvline(lam_best, color='r', linestyle='--', alpha=0.8,
                label=f'best λ = {lam_best:.3e}')
...
def run_stage_m2(image_data, noise_map, psf_kernel, feature_mask,
                  ...
                  position_likelihood, lambda_fixed: float):
    ...
    samples, weights, names, logz = _run_sampler(
        likelihood, n_live=300, n_eff=800, tag="stage-M2", vectorized=True,
    )
...
def main(skip_done: bool = False):
    ...
    if skip_done and (OUT_DIR / "stage_m1.pkl").exists():
        ...
        d = _load_stage("m1")
        lambda_m1 = d["extra"]["lambda_best"]
    else:
        lambda_m1 = run_stage_m1(
            image_data, noise_map, psf_kernel, feature_mask,
            lens_light_model, medians_a, position_likelihood,
        )

    if lambda_m1 is None:
        raise RuntimeError(
