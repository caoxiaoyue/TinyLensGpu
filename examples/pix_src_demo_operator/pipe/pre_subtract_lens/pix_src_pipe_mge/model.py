"""
Adaptive-regularization inference pipeline for the pix_src_pipe_mge demo.
    ...
    with ck.ActiveContext(likelihood):
        ...
        if not bool(np.asarray(solver_info.converged)):
            ...
            raise RuntimeError(
                ...
                f"stage source template ({metric_label}={metric_value:.4e}, "
...
def run_stage_a(image_data, noise_map, psf_kernel, circular_mask=None):
    ...
    _dump_stage(
        ...
        extra=dict(medians=medians, lens_light_model=lens_image_model, time_taken=t1 - t0),
        stage=stage,
    )
    ...
    try:
        plot_model_results(
            ...
            show_critical_lines=True, show_caustics=True,
        )
...
def build_stage_l_likelihood(
    ...
    phys = PhysicalModel(lens_mass=[], source_light=[], lens_light=lens_gaussians)
    return ImageProbModel(
        image_data=image_data, noise_map=noise_map_soft,
        psf_kernel=psf_kernel, dpix=DPIX, nsub=NSUB,
        phys_model=phys, use_linear=True, solver_type="nnls",
        mask=circular_mask,
    )
...
def run_stage_l(image_data, noise_map, psf_kernel, feature_mask,
    ...
    likelihood = build_stage_l_likelihood(
        image_data, noise_map, psf_kernel, feature_mask,
        stage_a, circular_mask=circular_mask,
    )
    stage = _run_sampler(
    ...
    # True-vs-fit lens light diagnostic (when ground truth available)
    try:
...
def _position_likelihood_from_stage_a(medians_a: dict) -> dict:
    ...
    required = (
        "theta_E", "e1_mass", "e2_mass", "center_x_mass", "center_y_mass",
        "gamma1", "gamma2", "center_x_src", "center_y_src",
    )
    missing = [k for k in required if k not in medians_a]
    if missing:
        raise KeyError("Stage-A medians missing required keys for position likelihood: " + ", ".join(missing))
    mass = PhysicalModel(
        lens_mass=[
            SIE(theta_E=float(medians_a["theta_E"]), e1=float(medians_a["e1_mass"]),
    ...
    solver = PointSourceProbModel(
        ...
        solver_config={"initial_range": 3.0, "n_x": 200, "n_y": 200, "k_keep": 30,
                       "num_iters": 20, "tolerance": 5.0e-4, "cluster_tol": 0.08},
    )
    ...
    return dict(positions=img_positions.tolist(), threshold_arcsec=float(threshold_arcsec), min_log_like=float(min_log_like))
...
def _sie_mass_from_stage_a(stage_a: StagePosterior):
    sie = SIE(
        theta_E=stage_a.fixed("theta_E"), e1=stage_a.fixed("e1_mass"),
        e2=stage_a.fixed("e2_mass"), center_x=stage_a.fixed("center_x_mass"),
        center_y=stage_a.fixed("center_y_mass"),
    )
    shear = Shear(gamma1=stage_a.fixed("gamma1"), gamma2=stage_a.fixed("gamma2"))
    return sie, shear
...
def _epl_mass_from_stage(stage: StagePosterior):
    ...
    shear = Shear(
        gamma1=stage.gaussian("gamma1", model="Shear", attr="gamma1", limits=[-0.5, 0.5]),
...
def build_stage_m0_likelihood(lens_subtracted_image, noise_map, psf_kernel, feature_mask,
    ...
    log_lam.to_dynamic()
    pix_src = PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE)
    phys = PhysicalModel(lens_mass=[sie, shear], source_light=[pix_src], lens_light=[])
...
def run_stage_m0(image_data, noise_map, psf_kernel, feature_mask,
    ...
    if not np.any(valid_coarse):
        raise RuntimeError("[stage-M0] All log-λ values in coarse grid failed.")
    ...
    if not np.any(valid_fine):
        raise RuntimeError("[stage-M0] All log-λ values in refinement grid failed.")
...
# Stage M1
# ------------------------------------------------------------------ #
def build_stage_m1_likelihood(lens_subtracted_image, noise_map, psf_kernel, feature_mask,
    ...
    log_lam.to_dynamic()
    pix_src = PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE)
    phys = PhysicalModel(lens_mass=[epl, shear], source_light=[pix_src], lens_light=[])
    combined_mask = feature_mask
    if circular_mask is not None:
        combined_mask = combined_mask | circular_mask
    return PixelizedImageProbModelOperator(
...
def run_stage_m1(image_data, noise_map, psf_kernel, feature_mask,
    ...
    print("=" * 60)
    print(f"[stage-M1] lambda_reg prior from M0: truncated Gaussian centered at {float(jnp.exp(log_lambda_fixed)):.4e}")
    t0 = time.time()
    lens_subtracted = image_data - lens_light_model
    likelihood = build_stage_m1_likelihood(
        lens_subtracted, noise_map, psf_kernel, feature_mask,
        stage_a, position_likelihood, log_lambda_fixed, circular_mask=circular_mask,
    )
    ...
    _dump_stage("m1", samples, weights, names, logz,
                extra=dict(medians=medians, m1_mass_model="EPL+Shear",
                           ...
                           s1=s1_package, time_taken=t1 - t0),
                stage=stage)
    try:
        _plot_pix_stage("stage-M1", likelihood, medians, names, str(OUT_DIR / "stage_m1_model.png"))
    except Exception as err:
...
def _plot_pix_stage(tag, likelihood, medians, param_names, save_path):
    ...
    with ck.ActiveContext(likelihood):
        ...
        n_s = likelihood.sim_obj.source_n; bs = likelihood.block_size
    ...
    model_image = np.zeros(image_data.shape); model_image[~mask] = model_1d
    ...
    for ax, img, title, kw in [
        ...
        ax.set_title(title, fontsize=11); ax.set_xlabel("arcsec")
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    ...
    axes[3].set_xlabel("arcsec"); plt.colorbar(im3, ax=axes[3], fraction=0.046, pad=0.04)
    im4 = axes[4].imshow(scale_img, origin="lower", extent=ext_s, cmap="plasma",
                         vmin=1.0, vmax=max(1.0, float(np.nanmax(scale_img))))
    axes[4].set_title(f"Reg precision scale\n(rho={rho_val:.2f})", fontsize=11)
...
def build_stage_m2_likelihood(lens_subtracted_image, noise_map, psf_kernel, feature_mask,
                                medians_m1: dict, position_likelihood, s1_package, circular_mask=None):
    epl, shear = _epl_mass_from_medians(medians_m1)
    log_lam = ParamU("log_lambda_reg", float(s1_package["log_lambda_best"]),
                     prior_type="uniform", prior_settings=[-13.815510557964274, 13.815510557964274],
                     limits=[-13.815510557964274, 13.815510557964274])
    log_lam.to_dynamic()
    rho = ParamU("adaptive_reg_rho", ADAPTIVE_REG_RHO, prior_type="uniform",
                 prior_settings=[0.0, ADAPTIVE_REG_RHO_PRIOR_MAX], limits=[0.0, ADAPTIVE_REG_RHO_PRIOR_MAX])
    rho.to_dynamic()
    pix_src = PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE, adaptive_reg_rho=rho)
    phys = PhysicalModel(lens_mass=[epl, shear], source_light=[pix_src], lens_light=[])
    combined_mask = feature_mask
    if circular_mask is not None:
        combined_mask = combined_mask | circular_mask
...
def run_stage_m2(image_data, noise_map, psf_kernel, feature_mask,
                  lens_light_model, medians_m1, position_likelihood, s1_package, circular_mask=None):
    print("\n" + "=" * 60)
    ...
    likelihood = build_stage_m2_likelihood(
        lens_subtracted, noise_map, psf_kernel, feature_mask,
        medians_m1, position_likelihood, s1_package, circular_mask=circular_mask,
    )
    ...
    medians = stage.medians()
    reg_hyperparams_m2 = {"log_lambda_reg": float(medians["log_lambda_reg"]), "adaptive_reg_rho": float(medians["adaptive_reg_rho"])}
    print(f"[stage-M2] median source reg: {_format_reg_hyperparams(reg_hyperparams_m2)}")
    _dump_stage("m2", samples, weights, names, logz,
                extra=dict(medians=medians, m2_mass_model="fixed-EPL+Shear",
                           mass_medians_fixed=dict(medians_m1), reg_hyperparams=reg_hyperparams_m2,
                           time_taken=t1 - t0), stage=stage)
    try:
        _plot_pix_stage("stage-M2", likelihood, medians, names, str(OUT_DIR / "stage_m2_model.png"))
    except Exception as err:
...
# Stage M3
# ------------------------------------------------------------------ #
def build_stage_m3_likelihood(lens_subtracted_image, noise_map, psf_kernel, feature_mask,
    ...
    log_lam = ParamU("log_lambda_reg", float(reg_hyperparams_fixed["log_lambda_reg"])); log_lam.to_static()
    rho = ParamU("adaptive_reg_rho", float(reg_hyperparams_fixed["adaptive_reg_rho"])); rho.to_static()
    pix_src = PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE, adaptive_reg_rho=rho)
    phys = PhysicalModel(lens_mass=[epl, shear], source_light=[pix_src], lens_light=[])
    combined_mask = feature_mask
    if circular_mask is not None:
...
def run_stage_m3(image_data, noise_map, psf_kernel, feature_mask,
                  lens_light_model, stage_m1: StagePosterior, position_likelihood,
    ...
    likelihood = build_stage_m3_likelihood(
        lens_subtracted, noise_map, psf_kernel, feature_mask,
        stage_m1, position_likelihood, reg_hyperparams_fixed, s1_package, circular_mask=circular_mask,
    )
    ...
    medians = stage.medians()
    _dump_stage("m3", samples, weights, names, logz,
                extra=dict(medians=medians, m3_mass_model="EPL+Shear",
                           reg_hyperparams_fixed=dict(reg_hyperparams_fixed), time_taken=t1 - t0),
                stage=stage)
    try:
        _plot_pix_stage("stage-M3", likelihood, medians, names, str(OUT_DIR / "stage_m3_model.png"))
    except Exception as err:
...
def main(skip_done: bool = False, out_dir: str | None = None):
    ...
    if skip_done and (OUT_DIR / "stage_a.pkl").exists():
        ...
        d = _load_stage("a"); stage_a = _stage_from_payload(d)
        medians_a = d["extra"]["medians"]; lens_light_model = d["extra"]["lens_light_model"]
    ...
    if skip_done and (OUT_DIR / "stage_l.pkl").exists():
        ...
        d = _load_stage("l"); stage_l = _stage_from_payload(d)
        medians_l = d["extra"]["medians"]; lens_light_model = d["extra"]["lens_light_model"]
        time_l = d["extra"].get("time_taken", 0.0)
    else:
        stage_l, medians_l, lens_light_model = run_stage_l(
            image_data, noise_map, psf_kernel, feature_mask, stage_a, circular_mask=circular_mask)
    ...
    if skip_done and (OUT_DIR / "stage_m1.pkl").exists():
        ...
        try:
            ...
            stage_m1, medians_m1, s1_package = run_stage_m1(
                image_data, noise_map, psf_kernel, feature_mask, lens_light_model,
                stage_a, position_likelihood, log_lambda_m0, circular_mask=circular_mask)
            names_m1 = stage_m1.param_names; time_m1 = _load_stage("m1")["extra"].get("time_taken", 0.0)
    else:
        stage_m1, medians_m1, s1_package = run_stage_m1(
            image_data, noise_map, psf_kernel, feature_mask, lens_light_model,
            stage_a, position_likelihood, log_lambda_m0, circular_mask=circular_mask)
    ...
    if s1_package is None:
        raise RuntimeError("[stage-M1] Failed to determine S1 source package.")
    if (OUT_DIR / "stage_m1.pkl").exists() and not (OUT_DIR / "stage_m1_model.png").exists():
        ...
        lkl = build_stage_m1_likelihood(lens_subtracted, noise_map, psf_kernel, feature_mask,
                                         stage_a, position_likelihood, log_lambda_m0, circular_mask=circular_mask)
        try:
            _plot_pix_stage("stage-M1", lkl, medians_m1, names_m1, str(OUT_DIR / "stage_m1_model.png"))
        except Exception as err:
            print(f"[stage-M1] re-plotting failed (non-fatal): {err}")

    # ---- stage M2 ---- #
    time_m2 = 0.0; reg_hyperparams_m2 = None
    if skip_done and (OUT_DIR / "stage_m2.pkl").exists():
        print(f"[stage-M2] loading cached {OUT_DIR}/stage_m2.pkl")
        d = _load_stage("m2"); stage_m2 = _stage_from_payload(d)
        names_m2 = stage_m2.param_names; medians_m2 = d["extra"]["medians"]
        reg_hyperparams_m2 = _reg_hyperparams_from_m2_payload(d); time_m2 = d["extra"].get("time_taken", 0.0)
    else:
        stage_m2, medians_m2, reg_hyperparams_m2 = run_stage_m2(
            image_data, noise_map, psf_kernel, feature_mask, lens_light_model,
            medians_m1, position_likelihood, s1_package, circular_mask=circular_mask)
        names_m2 = stage_m2.param_names; time_m2 = _load_stage("m2")["extra"].get("time_taken", 0.0)
    if not (OUT_DIR / "stage_m2_model.png").exists():
        lkl = build_stage_m2_likelihood(lens_subtracted, noise_map, psf_kernel, feature_mask,
                                         medians_m1, position_likelihood, s1_package, circular_mask=circular_mask)
        try:
            _plot_pix_stage("stage-M2", lkl, medians_m2, names_m2, str(OUT_DIR / "stage_m2_model.png"))
        except Exception as err:
            print(f"[stage-M2] plotting failed (non-fatal): {err}")
    if reg_hyperparams_m2 is None:
        raise RuntimeError("[stage-M2] Failed to determine source regularization hyperparameters.")

    # ---- stage M3 ---- #
    time_m3 = 0.0
    if skip_done and (OUT_DIR / "stage_m3.pkl").exists():
        print(f"[stage-M3] loading cached {OUT_DIR}/stage_m3.pkl")
        d = _load_stage("m3"); stage_m3 = _stage_from_payload(d)
        names_m3 = stage_m3.param_names; medians_m3 = d["extra"]["medians"]
        time_m3 = d["extra"].get("time_taken", 0.0)
    else:
        stage_m3, medians_m3 = run_stage_m3(
            image_data, noise_map, psf_kernel, feature_mask, lens_light_model,
            stage_m1, position_likelihood, reg_hyperparams_m2, s1_package, circular_mask=circular_mask)
        names_m3 = stage_m3.param_names; time_m3 = _load_stage("m3")["extra"].get("time_taken", 0.0)
    if not (OUT_DIR / "stage_m3_model.png").exists():
        lkl = build_stage_m3_likelihood(lens_subtracted, noise_map, psf_kernel, feature_mask,
                                         stage_m1, position_likelihood, reg_hyperparams_m2, s1_package,
                                         circular_mask=circular_mask)
        try:
            _plot_pix_stage("stage-M3", lkl, medians_m3, names_m3, str(OUT_DIR / "stage_m3_model.png"))
        except Exception as err:
            print(f"[stage-M3] plotting failed (non-fatal): {err}")
    ...
    print("=" * 60)
    print(" Time summary:")
    print(f"    Stage A:  {time_a/60:.2f} min"); print(f"    Stage L:  {time_l/60:.2f} min")
    print(f"    Stage M0: {time_m0/60:.2f} min"); print(f"    Stage M1: {time_m1/60:.2f} min")
    print(f"    Stage M2: {time_m2/60:.2f} min"); print(f"    Stage M3: {time_m3/60:.2f} min")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-done", action="store_true", help=f"Re-use cached posteriors in {OUT_DIR}/stage_*.pkl")
    parser.add_argument("--out-dir", default=None, help="Output directory relative to this script.")
