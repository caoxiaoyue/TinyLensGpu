"""
Adaptive-regularization inference pipeline for the pix_src_pipe_sersic demo.
...
def _dump_stage(tag: str, samples, weights, param_names, log_z, extra=None, stage=None):
    ...
    if stage is not None:
        ...
        payload = dict(
            samples=stage_payload["samples"], weights=stage_payload["weights"],
            param_names=stage_payload["param_names"], log_z=stage_payload["log_z"],
            stage_schema={key: stage_payload[key] for key in ("param_names", "prior_specs") if key in stage_payload},
            extra=extra or {},
        )
    else:
...
def _source_axes_from_bbox(source_bbox, n=NSRC):
    ...
    return (np.linspace(xmin, xmax, int(n), dtype=np.float64),
...
def _validate_s0_package(s0_package):
    ...
    if legacy_keys:
        raise KeyError(f"S0 package uses legacy source-grid keys {', '.join(legacy_keys)}")
...
def _fista_kwargs():
    return dict(fista_max_iter=FISTA_MAX_ITER, fista_rtol=FISTA_RTOL,
                fista_power_iter=FISTA_POWER_ITER, fista_step_safety=FISTA_STEP_SAFETY)
...
def _reg_hyperparams_from_payload(stage_payload, tag):
    ...
    if missing:
        raise KeyError(f"stage-{tag.upper()} medians missing required keys: " + ", ".join(missing))
...
def _format_reg_hyperparams(reg_hyperparams):
    return f"lambda={float(jnp.exp(reg_hyperparams['log_lambda_reg'])):.4e}, rho={reg_hyperparams['adaptive_reg_rho']:.4f}"
...
def _solve_pixel_source_for_package(likelihood, medians, param_names):
    ...
    with ck.ActiveContext(likelihood):
        ...
        op_data = likelihood.sim_obj.precompute_operator_data(xmin, xmax, ymin, ymax, _betas_sub=(beta_x_sub, beta_y_sub))
        block_chols, block_masks = likelihood.sim_obj.build_block_diag_preconditioner(
            ...
            likelihood.reg_builder, block_size=likelihood.block_size, scale=scale)
        source_pixels, solver_info = likelihood._solve_source(
            xmin, xmax, ymin, ymax, lambda_j, reg_data, (block_chols, block_masks), op_data=op_data)
        if not bool(np.asarray(solver_info.converged)):
            if hasattr(solver_info, "residual_norm"):
                metric_label = "residual"; metric_value = float(solver_info.residual_norm)
            else:
                metric_label = "convergence_metric"; metric_value = float(solver_info.convergence_metric)
            raise RuntimeError(f"{likelihood.solver_type.upper()} failed ({metric_label}={metric_value:.4e}, n_iter={int(solver_info.n_iter)}).")
    source_bbox = (float(xmin), float(xmax), float(ymin), float(ymax))
    x_axis, y_axis = _source_axes_from_bbox(source_bbox, NSRC)
    return dict(source_pixels=np.asarray(source_pixels, dtype=np.float64),
                source_image=np.asarray(source_pixels, dtype=np.float64).reshape(NSRC, NSRC),
                source_bbox=source_bbox, source_x_axis=x_axis, source_y_axis=y_axis, n=NSRC)
...
def build_stage_a_likelihood(image_data, noise_map, psf_kernel, mask=None):
    ...
    for p in (sie.theta_E, sie.e1, sie.e2, sie.center_x, sie.center_y, shear.gamma1, shear.gamma2):
        p.to_dynamic()

    # Single-Sersic source
    R_s_src = ParamU("R_sersic_src", 0.3, prior_type="uniform", prior_settings=[0.05, 1.5], limits=[0.01, 3.0])
    n_s_src = ParamU("n_sersic_src", 2.0, prior_type="uniform", prior_settings=[0.5, 8.0], limits=[0.3, 10.0])
    ...
    source_sersic = SersicEllipse(R_sersic=R_s_src, n_sersic=n_s_src, e1=e1_s, e2=e2_s, center_x=cx_s, center_y=cy_s, Ie=1.0)
    for p in (R_s_src, n_s_src, cx_s, cy_s, e1_s, e2_s):
        p.to_dynamic()

    # Single-Sersic lens light
    R_s_lens = ParamU("R_sersic_lens", 1.0, prior_type="uniform", prior_settings=[0.1, 3.0], limits=[0.05, 5.0])
    n_s_lens = ParamU("n_sersic_lens", 3.0, prior_type="uniform", prior_settings=[0.5, 8.0], limits=[0.3, 10.0])
    ...
    lens_sersic = SersicEllipse(R_sersic=R_s_lens, n_sersic=n_s_lens, e1=e1_l, e2=e2_l, center_x=cx_l, center_y=cy_l, Ie=1.0)
    for p in (R_s_lens, n_s_lens, cx_l, cy_l, e1_l, e2_l):
        p.to_dynamic()

    phys = PhysicalModel(lens_mass=[sie, shear], source_light=[source_sersic], lens_light=[lens_sersic])
    return ImageProbModel(image_data=image_data, noise_map=noise_map, psf_kernel=psf_kernel,
                           dpix=DPIX, nsub=NSUB, phys_model=phys, use_linear=True, solver_type="nnls", mask=mask)
...
def run_stage_a(image_data, noise_map, psf_kernel, circular_mask=None):
    ...
    try:
        plot_model_results(likelihood, jnp.asarray(q50), save_path=str(OUT_DIR / "stage_a_model.png"),
                          title="Stage A : Sersic lens+source", show_critical_lines=True, show_caustics=True)
...
def run_stage_b(image_data, noise_map, lens_light_model, circular_mask=None):
    ...
    arc_mask = arc_mask_from(snr_map, threshold=2.0, ignor_size=25, ext_size=2, close_size=3)
    ...
    fits.writeto(DATA_DIR / "feature_mask.fits", feature_mask.astype(np.uint8), overwrite=True)
    ny_img, nx_img = image_data.shape
    extent = [-nx_img * DPIX / 2, nx_img * DPIX / 2, -ny_img * DPIX / 2, ny_img * DPIX / 2]
    fig, axes = plt.subplots(1, 2, figsize=(9, 4))
    axes[0].imshow(residual, origin="lower", extent=extent, cmap="viridis"); axes[0].set_title("residual = image - stage-A lens light")
    axes[1].imshow(snr_map, origin="lower", extent=extent, cmap="viridis", vmin=-3, vmax=np.nanpercentile(snr_map, 99.5))
    axes[1].set_title("S/N map + arc mask boundary")
    axes[1].contour(~feature_mask, levels=[0.5], origin="lower", extent=extent, colors="red", linewidths=1.5)
...
def build_stage_l_likelihood(image_data, noise_map, psf_kernel, feature_mask,
    ...
    R_s = stage_a.gaussian("R_sersic_lens", model="Sersic", attr="R_sersic", limits=[0.05, 5.0])
    n_s = stage_a.gaussian("n_sersic_lens", model="Sersic", attr="n_sersic", limits=[0.3, 10.0])
    e1_l = ParamU("e1_lens", 0.0, prior_type="uniform", prior_settings=[-0.3, 0.3], limits=[-1.0, 1.0])
    e2_l = ParamU("e2_lens", 0.0, prior_type="uniform", prior_settings=[-0.3, 0.3], limits=[-1.0, 1.0])
    cx_l = stage_a.gaussian("center_x_lens", model="Sersic", attr="center_x", limits=[-1.0, 1.0])
    cy_l = stage_a.gaussian("center_y_lens", model="Sersic", attr="center_y", limits=[-1.0, 1.0])
    lens_sersic = SersicEllipse(R_sersic=R_s, n_sersic=n_s, e1=e1_l, e2=e2_l, center_x=cx_l, center_y=cy_l, Ie=1.0)
    for p in (R_s, n_s, cx_l, cy_l, e1_l, e2_l):
        p.to_dynamic()
    noise_map_soft = np.array(noise_map, copy=True)
    noise_map_soft[~feature_mask] *= 1000.0
    phys = PhysicalModel(lens_mass=[], source_light=[], lens_light=[lens_sersic])
    return ImageProbModel(image_data=image_data, noise_map=noise_map_soft, psf_kernel=psf_kernel,
                           dpix=DPIX, nsub=NSUB, phys_model=phys, use_linear=True, solver_type="nnls", mask=circular_mask)
...
def run_stage_l(image_data, noise_map, psf_kernel, feature_mask,
    ...
    print("=" * 60)
    t0 = time.time()
    n_arc = int((~feature_mask).sum()); n_nonarc = feature_mask.size - n_arc
    ...
    stage = _run_sampler(likelihood, n_live=1000, n_eff=2000, tag="stage-L", vectorized=True)
    t1 = time.time()
    samples, weights, names, logz = stage.samples, stage.weights, stage.param_names, stage.log_z
    ...
    # True-vs-fit lens light diagnostic
    try:
        lens_light_true = np.asarray(fits.getdata(DATA_DIR / "lens_light_true.fits"))
        diff = lens_light_true - lens_image_model; diff_norm = diff / noise_map
...
def _position_likelihood_from_stage_a(medians_a: dict) -> dict:
    ...
    if missing:
        raise KeyError("Stage-A medians missing required keys: "+", ".join(missing))
    mass = PhysicalModel(
        lens_mass=[SIE(theta_E=float(medians_a["theta_E"]), e1=float(medians_a["e1_mass"]),
    ...
    solver = PointSourceProbModel(
        phys_model=mass, observed_positions=[[0.0, 0.0]], position_sigma=[0.01],
    ...
    if img_positions.ndim != 2 or img_positions.shape[1] != 2:
        raise RuntimeError(f"Lens-equation solver returned invalid image_positions shape={img_positions.shape}")
    if img_positions.shape[0] < 2:
        raise RuntimeError("Lens-equation solver returned fewer than 2 lensed image positions.")
    print(f"[pos-like] solved {img_positions.shape[0]} lensed image positions from stage-A medians")
    for p in img_positions: print(f"    ({p[0]:+.4f}, {p[1]:+.4f})")
    return dict(positions=img_positions.tolist(), threshold_arcsec=0.3, min_log_like=-1.0e10)
...
def _sie_mass_from_stage_a(stage_a: StagePosterior):
    sie = SIE(theta_E=stage_a.fixed("theta_E"), e1=stage_a.fixed("e1_mass"), e2=stage_a.fixed("e2_mass"),
              center_x=stage_a.fixed("center_x_mass"), center_y=stage_a.fixed("center_y_mass"))
    return sie, Shear(gamma1=stage_a.fixed("gamma1"), gamma2=stage_a.fixed("gamma2"))


def _epl_mass_from_stage(stage: StagePosterior):
    theta_E = stage.gaussian("theta_E", model="EPL", attr="theta_E", limits=[0.0, 5.0])
    ...
    epl = EPL(theta_E=theta_E, gamma=gamma, e1=e1m, e2=e2m, center_x=cx, center_y=cy); epl.gamma.to_dynamic()
    shear = Shear(gamma1=stage.gaussian("gamma1", model="Shear", attr="gamma1", limits=[-0.5, 0.5]),
...
def _plot_pix_stage(tag, likelihood, medians, param_names, save_path):
    q50 = [medians[n] for n in param_names]
    image_data = np.asarray(likelihood.image_data); noise_map = np.asarray(likelihood.noise_map)
    mask = ~np.asarray(likelihood.unmask, dtype=bool)
    ...
    with ck.ActiveContext(likelihood):
        ...
        rho_val = float(np.asarray(_source_param_value(likelihood.phys_model.source_light[0].adaptive_reg_rho)))
        (xmin, xmax, ymin, ymax, beta_x_sub, beta_y_sub, beta_x_seed, beta_y_seed) = likelihood._get_bbox()
        scale = likelihood._get_reg_scale()
        reg_data = likelihood._regularization_data(xmin, xmax, ymin, ymax, scale=scale)
        op_data = likelihood.sim_obj.precompute_operator_data(xmin, xmax, ymin, ymax, _betas_sub=(beta_x_sub, beta_y_sub))
        block_chols, block_masks = likelihood.sim_obj.build_block_diag_preconditioner(
            likelihood.noise_1d, xmin, xmax, ymin, ymax, lambda_j, likelihood.reg_builder, block_size=likelihood.block_size, scale=scale)
        source_pixels, _ = likelihood._solve_source(xmin, xmax, ymin, ymax, lambda_j, reg_data, (block_chols, block_masks), op_data=op_data)
        model_1d_j = likelihood.sim_obj.forward_model(source_pixels, xmin, xmax, ymin, ymax, op_data=op_data)
        n_s = likelihood.sim_obj.source_n; bs = likelihood.block_size; n_blocks = (n_s+bs-1)//bs
        trace_invPR = jnp.array(0.0, dtype=lambda_j.dtype)
        for by in range(n_blocks):
            for bx in range(n_blocks):
                bid = bx+by*n_blocks; x_s,x_e = bx*bs, min((bx+1)*bs, n_s); y_s,y_e = by*bs, min((by+1)*bs, n_s)
                if bid >= len(block_chols): break
                R_block = likelihood.reg_builder.block_diag_R(x_s,x_e,y_s,y_e,xmin,xmax,ymin,ymax, scale=scale)
                trace_invPR = trace_invPR + jnp.trace(jsl.cho_solve((block_chols[bid], True), R_block))
        ...
        if has_pos_penalty:
            ...
            dx = beta_x[:,None]-beta_x[None,:]; dy = beta_y[:,None]-beta_y[None,:]
            max_sep = float(jnp.max(jnp.sqrt(dx*dx+dy*dy)))
            ...
            beta_x_np=np.array(beta_x); beta_y_np=np.array(beta_y)
            pos_px_np=np.array(likelihood._pos_px); pos_py_np=np.array(likelihood._pos_py)
    model_1d = np.array(model_1d_j); model_image = np.zeros(image_data.shape); model_image[~mask] = model_1d
    resid_norm = (image_data-model_image)/noise_map
    chi2 = float(np.sum(resid_norm[~mask]**2)); dof = int((~mask).sum())-N_eff; chi2_nu = chi2/dof if dof>0 else 0.0
    n = likelihood.phys_model.source_light[0].n; src_img = np.array(source_pixels).reshape(n,n)
    ...
    rows_unmasked, cols_unmasked = np.where(~mask); pad=3
    row_min=max(rows_unmasked.min()-pad,0); row_max=min(rows_unmasked.max()+pad,npix-1)
    col_min=max(cols_unmasked.min()-pad,0); col_max=min(cols_unmasked.max()+pad,npix-1)
    xlim=(-npix*DPIX/2+col_min*DPIX, -npix*DPIX/2+(col_max+1)*DPIX)
    ylim=(-npix*DPIX/2+row_min*DPIX, -npix*DPIX/2+(row_max+1)*DPIX)
    for ax, img, title, kw in [
        ...
        (axes[2], np.where(mask, np.nan, resid_norm), f"Norm. residual\nχ²/ν={chi2_nu:.3f}", dict(vmin=-3, vmax=3, cmap="RdBu_r")),
    ]:
        im = ax.imshow(img, origin="lower", extent=ext_i, **kw)
        if ax==axes[0] and has_pos_penalty: ax.plot(pos_px_np, pos_py_np, 'rx', markersize=8, label='Marked pos'); ax.legend(loc='upper right', fontsize=8)
        ax.set_title(title, fontsize=11); ax.set_xlabel("arcsec"); plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    im3 = axes[3].imshow(src_img, origin="lower", extent=ext_s, cmap="viridis")
    if has_pos_penalty: axes[3].plot(beta_x_np, beta_y_np, 'rx', markersize=8, label='Traced pos'); axes[3].legend(loc='upper right', fontsize=8)
    axes[3].set_title(f"Source reconstruction\n(λ={float(lam_val):.2e})", fontsize=11); axes[3].set_xlabel("arcsec")
    plt.colorbar(im3, ax=axes[3], fraction=0.046, pad=0.04)
    im4 = axes[4].imshow(scale_img, origin="lower", extent=ext_s, cmap="plasma", vmin=1.0, vmax=max(1.0,float(np.nanmax(scale_img))))
    axes[4].set_title(f"Reg precision scale\n(rho={rho_val:.2f})", fontsize=11); axes[4].set_xlabel("arcsec")
    plt.colorbar(im4, ax=axes[4], fraction=0.046, pad=0.04, label=r"$\lambda_i / \lambda_{\rm global}$")
    axes[0].set_ylabel("arcsec")
    for ax in axes[:3]: ax.set_xlim(*xlim); ax.set_ylim(*ylim)
...
# ------------------------------------------------------------------ #
# Stage M0-M3 (identical structure to slacs, adapted for Sersic pipeline)
# ------------------------------------------------------------------ #
def build_stage_m0_likelihood(lens_subtracted_image, noise_map, psf_kernel, feature_mask,
                                stage_a, position_likelihood, circular_mask=None):
    sie, shear = _sie_mass_from_stage_a(stage_a)
    log_lam = ParamU("log_lambda_reg", 0.0, prior_type="uniform",
                     prior_settings=[-13.815510557964274, 13.815510557964274], limits=[-13.815510557964274, 13.815510557964274])
    log_lam.to_dynamic()
    phys = PhysicalModel(lens_mass=[sie, shear], source_light=[PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE)], lens_light=[])
    cm = feature_mask;
    if circular_mask is not None: cm = cm | circular_mask
    return PixelizedImageProbModelOperator(image_data=lens_subtracted_image, noise_map=noise_map, psf_kernel=psf_kernel,
                                            dpix=DPIX, nsub=NSUB_PIX, phys_model=phys, mask=cm,
                                            position_likelihood=position_likelihood, solver_type=SOLVER_TYPE,
                                            source_bbox_padding=SOURCE_BBOX_PADDING, **_fista_kwargs())

def run_stage_m0(image_data, noise_map, psf_kernel, feature_mask, lens_light_model,
                  stage_a, position_likelihood, circular_mask=None):
    print("\n" + "=" * 60); print(" Stage M0 : fixed SIE + shear + uniform pix source (build S0)"); print("=" * 60)
    t0 = time.time(); ls = image_data - lens_light_model
    likelihood = build_stage_m0_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_a, position_likelihood, circular_mask=circular_mask)
    loglike_batch = make_likelihood(likelihood, vectorized=True); n_grid = 200
    ...
    vc = _valid_log_evidence(log_ev_coarse)
    if not np.any(vc): raise RuntimeError("[stage-M0] All coarse grid values failed.")
    best_idx = int(np.argmax(np.where(vc, np.asarray(log_ev_coarse), -np.inf)))
    log_lam_bc = float(log_lam_grid_coarse[best_idx])
    print(f"[stage-M0] Coarse best: λ = {float(jnp.exp(log_lam_bc)):.4e}")
    hw = 0.5*jnp.log(10)
    log_lam_grid_fine = jnp.linspace(log_lam_bc-hw, log_lam_bc+hw, n_grid)
    log_ev_fine = jnp.asarray(loglike_batch(log_lam_grid_fine.reshape(-1, 1)))
    vf = _valid_log_evidence(log_ev_fine)
    if not np.any(vf): raise RuntimeError("[stage-M0] All refinement grid values failed.")
    best_idx_f = int(np.argmax(np.where(vf, np.asarray(log_ev_fine), -np.inf)))
    log_lam_best = float(log_lam_grid_fine[best_idx_f]); log_ev_best = float(log_ev_fine[best_idx_f])
    print(f"[stage-M0] Refined best: λ = {float(jnp.exp(log_lam_best)):.4e}")
    medians_a = stage_a.medians()
    s0_package = _solve_pixel_source_for_package(likelihood, {**medians_a, "log_lambda_reg": log_lam_best}, ["log_lambda_reg"])
    ...
    s0_package["scale_map"] = np.asarray(_make_s0_scale(s0_package), dtype=np.float32); _validate_s0_package(s0_package)
    t1 = time.time(); print(f"[stage-M0] time taken: {t1-t0:.2f} seconds")
    _dump_stage("m0", None, None, ["log_lambda_reg"], log_ev_best,
                extra=dict(log_lambda_best=log_lam_best, evidence_lambda_best=float(jnp.exp(log_lam_best)),
                           evidence_log_lambda_best=log_lam_best, s0=s0_package, time_taken=t1-t0,
    ...
    try: _plot_pix_stage("stage-M0", likelihood, {**medians_a, "log_lambda_reg": log_lam_best}, ["log_lambda_reg"], str(OUT_DIR / "stage_m0_model.png"))
    except Exception as err: print(f"[stage-M0] plotting failed (non-fatal): {err}")
    return s0_package, log_lam_best

def build_stage_m1_likelihood(ls_img, noise_map, psf_kernel, feature_mask, stage_a, position_likelihood, log_lam_fixed, circular_mask=None):
    epl, shear = _epl_mass_from_stage(stage_a)
    log_lam = ParamU("log_lambda_reg", float(log_lam_fixed), prior_type="truncated_gaussian",
                     prior_settings=[float(log_lam_fixed), 0.15], limits=[float(log_lam_fixed)-0.5, float(log_lam_fixed)+0.5])
    log_lam.to_dynamic()
    phys = PhysicalModel(lens_mass=[epl, shear], source_light=[PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE)], lens_light=[])
    cm = feature_mask;
    if circular_mask is not None: cm = cm | circular_mask
    return PixelizedImageProbModelOperator(image_data=ls_img, noise_map=noise_map, psf_kernel=psf_kernel,
                                            dpix=DPIX, nsub=NSUB_PIX, phys_model=phys, mask=cm,
                                            position_likelihood=position_likelihood, solver_type=SOLVER_TYPE,
                                            source_bbox_padding=SOURCE_BBOX_PADDING, **_fista_kwargs())

def run_stage_m1(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_a, position_likelihood, log_lam_fixed, circular_mask=None):
    print("\n" + "=" * 60); print(" Stage M1 : EPL + shear + non-adaptive pix source (mass fit)"); print("=" * 60)
    t0 = time.time(); ls = image_data - lens_light_model
    likelihood = build_stage_m1_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_a, position_likelihood, log_lam_fixed, circular_mask=circular_mask)
    stage = _run_sampler(likelihood, n_live=300, n_eff=600, tag="stage-M1", vectorized=True)
    t1 = time.time(); samples, weights, names, logz = stage.samples, stage.weights, stage.param_names, stage.log_z
    _print_summary("stage-M1", samples, weights, names); print(f"[stage-M1] time taken: {t1-t0:.2f} seconds")
    medians = stage.medians(); s1_medians = dict(medians); log_lam_m1 = float(medians["log_lambda_reg"])
    s1_package = _solve_pixel_source_for_package(likelihood, s1_medians, names)
    s1_package.update(lambda_best=float(jnp.exp(log_lam_m1)), log_lambda_best=log_lam_m1, stage_m1_medians=dict(medians))
    s1_package["scale_map"] = np.asarray(_make_s0_scale(s1_package), dtype=np.float32); _validate_s0_package(s1_package)
    _dump_stage("m1", samples, weights, names, logz,
                extra=dict(medians=medians, m1_mass_model="EPL+Shear", lambda_prior_center=float(log_lam_fixed),
                           lambda_prior_sigma=0.15,
                           lambda_prior_limits=[float(log_lam_fixed - 0.5), float(log_lam_fixed + 0.5)],
                           s1=s1_package, time_taken=t1-t0), stage=stage)
    try: _plot_pix_stage("stage-M1", likelihood, medians, names, str(OUT_DIR / "stage_m1_model.png"))
    except Exception as err: print(f"[stage-M1] plot failed (non-fatal): {err}")
    return stage, medians, s1_package

def build_stage_m2_likelihood(ls_img, noise_map, psf_kernel, feature_mask, medians_m1, position_likelihood, s1_package, circular_mask=None):
    epl, shear = _epl_mass_from_medians(medians_m1)
    log_lam = ParamU("log_lambda_reg", float(s1_package["log_lambda_best"]), prior_type="uniform",
                     prior_settings=[-13.815510557964274, 13.815510557964274], limits=[-13.815510557964274, 13.815510557964274]); log_lam.to_dynamic()
    rho = ParamU("adaptive_reg_rho", ADAPTIVE_REG_RHO, prior_type="uniform", prior_settings=[0.0, ADAPTIVE_REG_RHO_PRIOR_MAX], limits=[0.0, ADAPTIVE_REG_RHO_PRIOR_MAX]); rho.to_dynamic()
    phys = PhysicalModel(lens_mass=[epl, shear], source_light=[PixelizedSourceModel(n=NSRC, log_lambda_reg=log_lam, regularization_type=PIXEL_REGULARIZATION_TYPE, adaptive_reg_rho=rho)], lens_light=[])
    cm = feature_mask;
    if circular_mask is not None: cm = cm | circular_mask
    return PixelizedImageProbModelOperator(image_data=ls_img, noise_map=noise_map, psf_kernel=psf_kernel,
                                            dpix=DPIX, nsub=NSUB_PIX, phys_model=phys, mask=cm,
...
def run_stage_m2(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, medians_m1, position_likelihood, s1_package, circular_mask=None):
    print("\n" + "=" * 60); print(" Stage M2 : fixed EPL + shear + adaptive pix source (fit λ, rho)"); print("=" * 60)
    t0 = time.time(); ls = image_data - lens_light_model
    likelihood = build_stage_m2_likelihood(ls, noise_map, psf_kernel, feature_mask, medians_m1, position_likelihood, s1_package, circular_mask=circular_mask)
    stage = _run_sampler(likelihood, n_live=250, n_eff=600, tag="stage-M2", vectorized=True)
    t1 = time.time(); samples, weights, names, logz = stage.samples, stage.weights, stage.param_names, stage.log_z
    _print_summary("stage-M2", samples, weights, names); print(f"[stage-M2] time taken: {t1-t0:.2f} seconds")
    medians = stage.medians()
    rhm2 = {"log_lambda_reg": float(medians["log_lambda_reg"]), "adaptive_reg_rho": float(medians["adaptive_reg_rho"])}
    print(f"[stage-M2] median source reg: {_format_reg_hyperparams(rhm2)}")
    _dump_stage("m2", samples, weights, names, logz, extra=dict(medians=medians, m2_mass_model="fixed-EPL+Shear", mass_medians_fixed=dict(medians_m1), reg_hyperparams=rhm2, time_taken=t1-t0), stage=stage)
    try: _plot_pix_stage("stage-M2", likelihood, medians, names, str(OUT_DIR / "stage_m2_model.png"))
    except Exception as err: print(f"[stage-M2] plot failed (non-fatal): {err}")
    return stage, medians, rhm2

def build_stage_m3_likelihood(ls_img, noise_map, psf_kernel, feature_mask, stage_m1, position_likelihood, rh_fixed, s1_package, circular_mask=None):
    epl, shear = _epl_mass_from_stage(stage_m1)
    log_lam = ParamU("log_lambda_reg", float(rh_fixed["log_lambda_reg"])); log_lam.to_static()
...
def run_stage_m3(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_m1, position_likelihood, rh_fixed, s1_package, circular_mask=None):
    ...
    t0 = time.time(); ls = image_data - lens_light_model
    likelihood = build_stage_m3_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_m1, position_likelihood, rh_fixed, s1_package, circular_mask=circular_mask)
    stage = _run_sampler(likelihood, n_live=300, n_eff=600, tag="stage-M3", vectorized=True)
    t1 = time.time(); samples, weights, names, logz = stage.samples, stage.weights, stage.param_names, stage.log_z
    _print_summary("stage-M3", samples, weights, names); print(f"[stage-M3] time taken: {t1-t0:.2f} seconds")
    medians = stage.medians()
    _dump_stage("m3", samples, weights, names, logz, extra=dict(medians=medians, m3_mass_model="EPL+Shear", reg_hyperparams_fixed=dict(rh_fixed), time_taken=t1-t0), stage=stage)
    try: _plot_pix_stage("stage-M3", likelihood, medians, names, str(OUT_DIR / "stage_m3_model.png"))
    except Exception as err: print(f"[stage-M3] plot failed (non-fatal): {err}")
...
def main(skip_done=False, out_dir=None):
    ...
    image_data, noise_map, psf_kernel, _ = load_lens_data(image_path=str(DATA_DIR/"image.fits"), noise_path=str(DATA_DIR/"noise.fits"), psf_path=str(DATA_DIR/"psf.fits"))
    circular_mask = _make_circular_mask(image_data.shape, DPIX, radius_arcsec=MASK_RADIUS)
    print(f"[circular mask] excluded {int(circular_mask.sum())} / {circular_mask.size} pixels (> {MASK_RADIUS} arcsec)")

    ta=0.0
    if skip_done and (OUT_DIR/"stage_a.pkl").exists():
        print(f"[stage-A] loading cached {OUT_DIR}/stage_a.pkl"); d=_load_stage("a"); stage_a=_stage_from_payload(d); medians_a=d["extra"]["medians"]; lens_light_model=d["extra"]["lens_light_model"]; ta=d["extra"].get("time_taken",0.0)
    else: stage_a, medians_a, lens_light_model = run_stage_a(image_data, noise_map, psf_kernel, circular_mask=circular_mask); ta=_load_stage("a")["extra"].get("time_taken",0.0)
    ...
    if skip_done and (OUT_DIR/"stage_l.pkl").exists():
        print(f"[stage-L] loading cached {OUT_DIR}/stage_l.pkl"); d=_load_stage("l"); stage_l=_stage_from_payload(d); medians_l=d["extra"]["medians"]; lens_light_model=d["extra"]["lens_light_model"]; tl=d["extra"].get("time_taken",0.0)
    else: stage_l, medians_l, lens_light_model = run_stage_l(image_data, noise_map, psf_kernel, feature_mask, stage_a, circular_mask=circular_mask); tl=_load_stage("l")["extra"].get("time_taken",0.0)

    ls = image_data - lens_light_model; position_likelihood = _position_likelihood_from_stage_a(medians_a)

    tm0=0.0
    if skip_done and (OUT_DIR/"stage_m0.pkl").exists():
        print(f"[stage-M0] loading cached {OUT_DIR}/stage_m0.pkl"); d=_load_stage("m0"); s0_package=_validate_s0_package(d["extra"]["s0"]); log_lam_m0=d["extra"]["log_lambda_best"]; tm0=d["extra"].get("time_taken",0.0)
    else: s0_package, log_lam_m0 = run_stage_m0(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_a, position_likelihood, circular_mask=circular_mask); tm0=_load_stage("m0")["extra"].get("time_taken",0.0)
    if (OUT_DIR/"stage_m0.pkl").exists() and not (OUT_DIR/"stage_m0_model.png").exists():
        lkl=build_stage_m0_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_a, position_likelihood, circular_mask=circular_mask)
        try: _plot_pix_stage("stage-M0", lkl, {**medians_a, "log_lambda_reg": float(s0_package["log_lambda_best"])}, ["log_lambda_reg"], str(OUT_DIR/"stage_m0_model.png"))
        except Exception as err: print(f"[stage-M0] re-plot failed: {err}")

    tm1=0.0; s1_package=None
    if skip_done and (OUT_DIR/"stage_m1.pkl").exists():
        print(f"[stage-M1] loading cached {OUT_DIR}/stage_m1.pkl"); d=_load_stage("m1")
        try: names_m1=d["param_names"]; stage_m1=_stage_from_payload(d); medians_m1=d["extra"]["medians"]; s1_package=_validate_s0_package(d["extra"]["s1"]); tm1=d["extra"].get("time_taken",0.0)
        except KeyError as err:
            print(f"[stage-M1] old format ({err}); recomputing.")
            stage_m1, medians_m1, s1_package = run_stage_m1(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_a, position_likelihood, log_lam_m0, circular_mask=circular_mask)
            names_m1=stage_m1.param_names; tm1=_load_stage("m1")["extra"].get("time_taken",0.0)
    else: stage_m1, medians_m1, s1_package = run_stage_m1(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_a, position_likelihood, log_lam_m0, circular_mask=circular_mask); names_m1=stage_m1.param_names; tm1=_load_stage("m1")["extra"].get("time_taken",0.0)
    if s1_package is None: raise RuntimeError("[stage-M1] Failed to determine S1 source package.")
    if (OUT_DIR/"stage_m1.pkl").exists() and not (OUT_DIR/"stage_m1_model.png").exists():
        lkl=build_stage_m1_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_a, position_likelihood, log_lam_m0, circular_mask=circular_mask)
        try: _plot_pix_stage("stage-M1", lkl, medians_m1, names_m1, str(OUT_DIR/"stage_m1_model.png"))
        except Exception as err: print(f"[stage-M1] re-plot failed: {err}")

    tm2=0.0; rhm2=None
    if skip_done and (OUT_DIR/"stage_m2.pkl").exists():
        print(f"[stage-M2] loading cached {OUT_DIR}/stage_m2.pkl"); d=_load_stage("m2"); stage_m2=_stage_from_payload(d); names_m2=stage_m2.param_names; medians_m2=d["extra"]["medians"]; rhm2=_reg_hyperparams_from_m2_payload(d); tm2=d["extra"].get("time_taken",0.0)
    else: stage_m2, medians_m2, rhm2 = run_stage_m2(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, medians_m1, position_likelihood, s1_package, circular_mask=circular_mask); names_m2=stage_m2.param_names; tm2=_load_stage("m2")["extra"].get("time_taken",0.0)
    if not (OUT_DIR/"stage_m2_model.png").exists():
        lkl=build_stage_m2_likelihood(ls, noise_map, psf_kernel, feature_mask, medians_m1, position_likelihood, s1_package, circular_mask=circular_mask)
        try: _plot_pix_stage("stage-M2", lkl, medians_m2, names_m2, str(OUT_DIR/"stage_m2_model.png"))
    ...
    if skip_done and (OUT_DIR/"stage_m3.pkl").exists():
        print(f"[stage-M3] loading cached {OUT_DIR}/stage_m3.pkl"); d=_load_stage("m3"); stage_m3=_stage_from_payload(d); names_m3=stage_m3.param_names; medians_m3=d["extra"]["medians"]; tm3=d["extra"].get("time_taken",0.0)
    else: stage_m3, medians_m3 = run_stage_m3(image_data, noise_map, psf_kernel, feature_mask, lens_light_model, stage_m1, position_likelihood, rhm2, s1_package, circular_mask=circular_mask); names_m3=stage_m3.param_names; tm3=_load_stage("m3")["extra"].get("time_taken",0.0)
    if not (OUT_DIR/"stage_m3_model.png").exists():
        lkl=build_stage_m3_likelihood(ls, noise_map, psf_kernel, feature_mask, stage_m1, position_likelihood, rhm2, s1_package, circular_mask=circular_mask)
        try: _plot_pix_stage("stage-M3", lkl, medians_m3, names_m3, str(OUT_DIR/"stage_m3_model.png"))
    ...
    print(f"    M0 best lambda_reg     = {float(jnp.exp(log_lam_m0)):.4e}")
    ...
    for k in ("theta_E","gamma","e1_mass","e2_mass","center_x_mass","center_y_mass","gamma1","gamma2"):
        if k in medians_m3: print(f"    final  {k:15s} = {medians_m3[k]:+.4f}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--skip-done", action="store_true"); parser.add_argument("--out-dir", default=None)
    main(skip_done=parser.parse_args().skip_done, out_dir=parser.parse_args().out_dir)
