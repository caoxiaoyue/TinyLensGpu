...
This is the **dense matrix-backend** counterpart to ``model.py``.  It uses
:class:`~TinyLensGpu.ObservationModel.LensImage.pixelized_image_model.PixelizedImageProbModel`
(explicit design matrix + Cholesky/NNLS) for the pixelized source stages
(M1/M2).  All modelling assumptions, priors, masks, and grid-search logic are
kept identical to ``model.py``; only the pixelized-source backend is changed
from the operator/PCG implementation to the matrix implementation.
...
``output_matrix/stage_{a,m1,m2}.pkl`` and is re-runnable via ``--skip-done``.

Usage::

    # From no_lens_light/
    python model_matrix.py
    python model_matrix.py --skip-done
"""
...
OUT_DIR = Path("output_matrix")
DATA_DIR = Path("data")


# ------------------------------------------------------------------ #
def _run_sampler(likelihood, n_live: int, n_eff: int, tag: str,
                  vectorized: bool = True, n_batch: int = 100):
...
def _eval_grid_chunked(loglike_fn, grid, chunk_size: int = 10):
    """Evaluate a batched log-likelihood over ``grid`` in smaller chunks.

    The dense-matrix backend materialises the design matrix for each
    likelihood call, so a full ``vmap`` over 200 λ points can exhaust GPU
    memory.  Splitting into small batches keeps the memory footprint low
    while still benefiting from JIT compilation.
    """
    chunks = []
    n = len(grid)
    for start in range(0, n, chunk_size):
        end = min(start + chunk_size, n)
        chunk = grid[start:end].reshape(-1, 1)
        chunks.append(jnp.asarray(loglike_fn(chunk)))
...
def _dump_stage(tag: str, samples, weights, param_names, log_z, extra=None):
    ...
    print(f"[{tag}] posterior saved to output_matrix/stage_{tag}.pkl")
...
def build_stage_m1_likelihood(
    ...
    # Dense matrix backend: explicit design matrix + NNLS linear solver.
...
def run_stage_m1(image_data, noise_map, psf_kernel, feature_mask,
    ...
    log_ev_fine = _eval_grid_chunked(loglike_batch, log_lam_grid_fine, chunk_size=10)
...
def _plot_pix_stage(tag, likelihood, medians, param_names, save_path):
    ...
    This implementation uses the dense-matrix backend:
    ``sim_obj.design_matrix()`` builds the explicit (Nd, Ns) PSF-convolved
    mapping, and ``_solve_source`` performs the Cholesky/NNLS solve.
    """
    ...
    with ck.ActiveContext(likelihood):
        ...
        # Dense backend: explicit design matrix and regularization matrix.
        design_matrix, source_bbox = likelihood.sim_obj.design_matrix()
        reg_matrix, _ = likelihood._regularization_matrix(source_bbox)
        linear_params, chol, _ = likelihood._solve_source(
            design_matrix, reg_matrix, lambda_j,
        )
        source_pixels = linear_params
        model_1d_j = design_matrix @ source_pixels

        # Effective number of source parameters: Ns - λ * trace(A^{-1} R)
        A_inv_R = jsl.cho_solve((chol, True), reg_matrix)
        N_eff = float(reg_matrix.shape[0] - lambda_j * jnp.trace(A_inv_R))
...
def run_stage_m2(image_data, noise_map, psf_kernel, feature_mask,
    ...
    # Dense-matrix backend: evaluate one likelihood call at a time to keep
    # the PSF-convolved design matrix memory footprint minimal.
    samples, weights, names, logz = _run_sampler(
        likelihood, n_live=300, n_eff=600, tag="stage-M2",
        vectorized=True, n_batch=1,
    )
    t1 = time.time()
...
def main(skip_done: bool = False):
    ...
    if skip_done and (OUT_DIR / "stage_a.pkl").exists():
        print("[stage-A] loading cached output_matrix/stage_a.pkl")
        d = _load_stage("a")
    ...
    if skip_done and (OUT_DIR / "stage_m1.pkl").exists():
        print("[stage-M1] loading cached output_matrix/stage_m1.pkl")
        d = _load_stage("m1")
    ...
    if skip_done and (OUT_DIR / "stage_m2.pkl").exists():
        print("[stage-M2] loading cached output_matrix/stage_m2.pkl")
        d = _load_stage("m2")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-done", action="store_true",
                        help="Re-use cached posteriors in output_matrix/stage_*.pkl")
