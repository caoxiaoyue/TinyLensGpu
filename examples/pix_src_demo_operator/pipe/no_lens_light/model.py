"""
Adaptive-regularization inference pipeline for lensing data with pre-subtracted lens light.
...
Stage m1 : EPL + shear + non-adaptive pixelized source — Nautilus sampling
            of mass parameters with lambda_reg constrained around stage-M0, then
...
``output/stage_{a,m0,m1,m2,m3}.pkl`` and is re-runnable via ``--skip-done``.
...
Usage::
    ...
    python model.py --skip-done --out-dir output_myrun
"""
...
def run_stage_b(image_data, noise_map, circular_mask=None):
    ...
    axes[0].set_title("image_no_lens (lens-subtracted)")
